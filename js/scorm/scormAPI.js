...
/*function setCommand(command, args){
	var objScorm-1Obj = g_bIsInternetExplorer ? objScorm-1 : document.objScorm-1;
	...
	if (cmd.substring(0,3) == "LMS"){
		...
		if ( cmd == "LMSInitialize" ){
			...
			if ( cmd == "LMSGetValue") {
				objScorm-1Obj.SetVariable(arg2, SCOGetValue(arg1));
			}	else if ( cmd == "LMSGetLastError") {
				objScorm-1Obj.SetVariable(arg2, SCOGetLastError(arg1));
			}	else if ( cmd == "LMSGetErrorString") {
				objScorm-1Obj.SetVariable(arg2, SCOGetLastError(arg1));
			}	else if ( cmd == "LMSGetDiagnostic") {
				objScorm-1Obj.SetVariable(arg2, SCOGetDiagnostic(arg1));
			}	else {
				// for unknown LMSGetxxxx extension
				v = eval('g_objAPI.' + cmd + '(\"' + arg1 + '\")');
				objScorm-1Obj.SetVariable(arg2,v);
			}
		...
		// These are using AICC HACP data model conventions, so remap data from AICC to SCORM as necessary.
		...
		if (cmd=="CMIFinish" || cmd=="CMIExitAU"){
			...
			// If commands comes with a 2nd argument, assume a value is expected otherwise assume it is just a cmd
